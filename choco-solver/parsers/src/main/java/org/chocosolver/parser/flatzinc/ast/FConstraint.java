...
//                            model.arithm(a, "<=", b).reifyWith(r);
    ...
    minCumulativeChoco {
        @Override
        public void build(Model model, Datas datas, String id, List<Expression> exps, List<EAnnotation> annotations) {
            IntVar[] starts = exps.get(0).toIntVarArray(model);
            IntVar[] processingTimes = exps.get(1).toIntVarArray(model);
            IntVar[] heights = exps.get(2).toIntVarArray(model);
            int[] demand = exps.get(3).toIntArray();
            model.minCumulative(starts, processingTimes, heights, demand).post();
        }
    },
    cumulativeChoco {
        @Override
